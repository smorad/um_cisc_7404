...
// TODO: Learn bold(theta)_f (memory) through IL
// Assume frozen and RL only learns bold(theta)_pi (linear)
...
#let bimodal_pi = { 
    ...
    canvas(length: 1cm, 
    {
        ...
        plot.plot(
            ...
            content((1.5,5), text(fill: orange)[$ a_+ $])

            line((4.5,0), (4.5,4), stroke: orange)
            content((4.5,5), text(fill: orange)[$ a_- $])
    }
)}
...
#side-by-side[
    ...
    $ bold(bold(theta))_pi in bb(R)^(|A| times |S|) $
]
...
$#bluem[$a_t$] tilde pi (a | s_t; bold(theta)_pi)$ #pause
...
$s_t = f(#redm[$bold(tau)_t$], bold(theta)_f)$ 

$#bluem[$a_t$] tilde pi (a | s_t; bold(theta)_pi)$ 
...
$ underbrace(pi (a | s; bold(theta)_pi), "Learned policy") = underbrace(pi (a | s; bold(theta)_beta), "Human speech") $ #pause

$ argmin_(bold(theta)_pi) sum_(s in bold(X)) sum_(a in A) - underbrace(pi (a | s; bold(theta)_beta), "Human speech") log underbrace(pi (a | s; bold(theta)_pi), "Learned policy") $ #pause

*Question:* Am I missing anything? #pause Hint: Where does $s$ come from? #pause

*Answer:* Must learn $bold(theta)_f$ to find $s = f(bold(tau), bold(theta)_f)$

==
$ argmin_(bold(theta)_pi) sum_(s in bold(X)) sum_(a in A) - underbrace(pi (a | s; bold(theta)_beta), "Human speech") log underbrace(pi (a | s; bold(theta)_pi), "Learned policy") $ #pause

$ s_t = f(#pin(1)bold(tau)_t#pin(2), bold(theta)_f) $ #pause

#pinit-highlight-equation-from((1,2), (1,2), fill: red, pos: bottom, height: 1.5em)[$o_0, a_0, dots, o_t$] #pause

Plug in for $s$ and find both $bold(theta)_pi, bold(theta)_f$ #pause

#v(1.5em)

$ underbrace( argmin_(bold(theta)_pi, bold(theta)_f) sum_(bold(tau) in bold(X)) sum_(t=0)^n sum_(a in A) - pi (a | #pin(3)f(bold(tau)_t, bold(theta)_f )#pin(4) ; bold(theta)_beta) log #pin(5)pi (a | f(bold(tau)_t, bold(theta)_f ); bold(theta)_pi), #text(fill: orange)[Speak like human]) $ #pause

#pinit-highlight-equation-from((3,4), (3,4), fill: blue, pos: top, height: 1.5em)[Human thought] #pause

Model learns to #text(fill: blue)[think like human ($bold(theta)_f$)] and #text(fill: orange)[talk like human ($bold(theta)_pi$)]
...
*Answer:* Our human expert $bold(theta)_beta$ is a combination many "experts" #pause
...
$ argmin_(bold(theta)_pi, bold(theta)_f) sum_(bold(tau) in bold(X)_"SFT") sum_(t=0)^n sum_(a in A) - pi (a | f(bold(tau)_t, bold(theta)_f ) ; bold(theta)_beta) log pi (a | f(bold(tau)_t, bold(theta)_f ); bold(theta)_pi) $ #pause

*Variant 2:* Change how the model talks ($bold(theta)_f$ already known) #pause
...
$ cal(R)(f("I", bold(theta)_f)), cal(R)(f("I love", bold(theta)_f)), cal(R)(f("I love you", bold(theta)_f)) $ #pause
...
//$ A(s_0, bold(theta)_pi_-, bold(theta)_pi_+) = sigma( bb(E)[cal(G)(bold(tau)) | s_0, bold(theta)_pi_+] - bb(E)[cal(G)(bold(tau)) | s_0 ; bold(theta)_pi_-] ) $ 
...
$ argmax_(bold(theta)_pi) bb(E)[cal(G)(bold(tau)) | s_0; bold(theta)_pi] = underbrace(argmax_(bold(theta)_pi) sum_(bold(tau)_-, bold(tau)_+) rho(bold(tau)_-, bold(tau)_+), "Choose" a in bold(tau)_+ "not" a in bold(tau)_-) $ #pause
...
Offline RL learns to think ($bold(theta)_f$) and speak $bold(theta)_pi$ *better* than the expert $bold(theta)_beta$! #pause
...
= Inverse RL
==
In inverse RL, we learn the parameters for the reward function $bold(theta)_cal(R)$

$ cal(R)(s, bold(theta)_cal(R)) $ #pause

In RLHF, it is easier to learn the parameters for return $bold(theta)_cal(G)$

$ cal(G)(bold(tau), bold(theta)_cal(G)) $ #pause
...
$ rho(bold(tau)_-, bold(tau_+), bold(theta)_cal(G)) = sigma (cal(G)(bold(tau)_+, bold(theta)_cal(G)) - cal(G)(bold(tau)_-, bold(theta)_cal(G))) = Pr("Human prefers" bold(tau)_+ ) $ #pause

We want to find $bold(theta)_cal(G)$ that predicts the human preferences in our dataset #pause
...
- Learn $bold(theta)_cal(G)$ that make $cal(G)(bold(tau)_+, bold(theta)_cal(G))$ big and make $cal(G)(bold(tau)_-, bold(theta)_cal(G))$ small #pause
*/

$ bold(theta)_cal(G) = argmax_(bold(theta)_cal(G)) sum_(bold(tau)_+, bold(tau)_- in bold(X)) underbrace(rho(bold(tau)_-, bold(tau_+)), "Label from" bold(X))  log underbrace(sigma (cal(G)(bold(tau)_+, bold(theta)_cal(G)) - cal(G)(bold(tau)_-, bold(theta)_cal(G))), "Learn" bold(theta)_cal(G) "to match label") 
$
...
We often model $cal(G)(bold(tau), bold(theta)_cal(G))$ using another LLM #pause

$ cal(G)(bold(tau), bold(bold(theta))_cal(G)) = softmax(bold(bold(theta))_cal(G)^top #pin(1)f(bold(tau), bold(theta)_f)#pin(2)) $ #pause
...
$
bold(theta)_(pi, i+1) &= bold(theta)_(pi, i) + alpha dot nabla_(bold(theta)_(pi, i)) bb(E)[cal(G)(bold(tau)) | s_0; bold(theta)_(pi, i)] \  #pause

nabla_bold(theta)_(pi, i) bb(E)[cal(G)(bold(tau)) | s_0; bold(theta)_(pi, i)] &= bb(E)[cal(G)(bold(tau)) | s_0; bold(theta)_(pi, i)] dot nabla_bold(theta)_(pi, i) log pi (a_0 | s_0; bold(theta)_(pi, i))
$ #pause

Plug in our learned return function $cal(G)(bold(tau), bold(theta)_cal(G))$ #pause

$
bold(theta)_(pi, i+1) &= bold(theta)_(pi, i) + alpha dot nabla_(bold(theta)_(pi, i)) bb(E)[cal(G)(bold(tau), #redm[$bold(theta)_cal(G)$]) | s_0; bold(theta)_(pi, i)] \

nabla_bold(theta)_(pi, i) bb(E)[cal(G)(bold(tau), #redm[$bold(theta)_cal(G)$]) | s_0; bold(theta)_(pi, i)] &= bb(E)[cal(G)(bold(tau), #redm[$bold(theta)_cal(G)$]) | s_0; bold(theta)_(pi, i)] dot nabla_bold(theta)_(pi, i) log pi (a_0 | s_0; bold(theta)_(pi, i))
$ #pause
...
After learning $bold(theta)_cal(G)$, we can use normal RL #pause
...
3. Compute the return $cal(G)(bold(tau), bold(theta)_cal(G))$ #pause
4. Update LLM parameters $bold(theta)_pi, bold(theta)_f$ using $cal(G)(bold(tau), bold(theta)_cal(G))$ #pause
    - To simplify equations, I will only learn $bold(theta)_pi$ #pause
    - But you can also learn $bold(theta)_f, bold(theta)_pi$ together
...
$
bold(theta)_(pi, i+1) = bold(theta)_(pi, i) + alpha dot nabla_(bold(theta)_pi) bb(E)[cal(G)(bold(tau), bold(theta)_cal(G)) | s_0; bold(theta)_pi]
$
...
$
bold(theta)_(pi, i+1) = #pin(1)bold(theta)_(pi, i) + alpha dot nabla_(bold(theta)_pi)  
    bb(E)[cal(G)(bold(tau), bold(theta)_cal(G)) |#pin(2) s_0; bold(theta)_(pi, i)]#pin(3) \ - 
    #pin(4)nabla_(bold(theta)_(pi, i + 1)) KL[pi (a | s; bold(theta)_(beta)), pi (a | s; bold(theta)_(pi, i + 1))]#pin(5)
$ #pause
...
$ bold(tau)_i tilde Pr (bold(tau) | s_q; bold(theta)_pi) $

$ bold(theta)_(pi, i+1) &= bold(theta)_(pi, i) + alpha dot nabla_(bold(theta)_(pi, i)) bb(E)[cal(G)(bold(tau), bold(theta)_cal(G)) | s_q; bold(theta)_(pi, i)] $
...
$ bold(theta)_pi = argmin_(bold(theta)_pi) sum_(s_0 in bold(X)) sum_(a in A) - pi (a | s_0; bold(theta)_beta) log pi (a | s_0; bold(theta)_pi) dot exp(hat(bb(E))[cal(G)(bold(tau)) | s_0; bold(theta)_beta]) $ #pause

Recall that MARWIL uses the advantage instead of return

$ bold(theta)_pi = argmin_(bold(theta)_pi) sum_(s_0 in bold(X)) sum_(a in A) - pi (a | s_0; bold(theta)_beta) log pi (a | s_0; bold(theta)_pi) dot underbrace(exp(A(s, a, bold(theta)_beta) ), "Advantage weight") $ #pause
...
$ A(s, a, bold(theta)_beta) = underbrace(Q(s, a, bold(theta)_beta), "Specific action") - underbrace(V(s, bold(theta)_beta), "Policy action") $ #pause

$ Pr(s_0, a_+, a_-, bold(theta)_pi) &= softmax( 
    vec(
        bb(E)[cal(G)(bold(tau)) | s_0, a_-; bold(theta)_pi],
        bb(E)[cal(G)(bold(tau)) | s_0, a_+; bold(theta)_pi],
    )) \ &= 
    softmax(vec(
        Q(s_0, a_-, bold(theta)_pi),
        Q(s_0, a_+, bold(theta)_pi),
    )
) $
